...
async function heartbeatReportRaw(req, res) {
   ...
   let save = false;
   if (req?.params?.save == 'true') save = 'true';
   let result = formatData(req.body, save, 'measurement');
   //console.log(result);
...
function formatData(body, insert, valueKey) {
   let heartbeatData = valueKey == 'measurement' ? body?.clinical_data?.HEART_RATE?.data : body;
   let startTime = null, endTime, high = null, low = null;
   let result = [];
   let patient = [], report = [];
   for (let i = 0; i < heartbeatData.length; i++) {
      if (startTime == null || new Date(heartbeatData[i].on_date) > endTime) {
         if (i > 0) result.push({ from_date: startTime, to_date: new Date(heartbeatData[i - 1].on_date), measurement: { low: low.toString(), high: high.toString() } });
         startTime = new Date(heartbeatData[i].on_date);
         endTime = new Date(startTime.getTime() + 15 * 60000);
         high = parseInt(heartbeatData[i][valueKey]);
         low = parseInt(heartbeatData[i][valueKey]);
      } else {
         if (high < heartbeatData[i][valueKey]) high = parseInt(heartbeatData[i][valueKey]);
         // else if (high == null) high = req.body[i].measurement;
         if (low > heartbeatData[i][valueKey]) low = parseInt(heartbeatData[i][valueKey]);
         // else if (low == null) low = req.body[i].measurement;
      }
      if (i == heartbeatData.length - 1) result.push({ from_date: startTime, to_date: new Date(heartbeatData[i].on_date), measurement: { low: low.toString(), high: high.toString() } });
      if (insert) {
         /*patient.push({
             "patient_id": body.patient_id,
             "from_healthkit_sync": body.from_healthkit_sync,
             "orgId": body.orgId,
             "timestamp": body.timestamp ? new Date(heartbeatData.timestamp) : null
         });*/

         let temp = 'HEART_RATE';
         if (body?.clinical_data?.[temp]?.data?.[i]?.measurement) {
            report.push(reportDBFormat(body, i, temp));
         }
         temp = "WEIGHT";
         if (body?.clinical_data?.[temp]?.data?.[i]?.measurement) {
            report.push(reportDBFormat(body, i, temp));
         }
         temp = "BLOOD_GLUCOSE_LEVELS";
         if (body?.clinical_data?.[temp]?.data?.[i]?.measurement) {
            report.push(reportDBFormat(body, i, temp));
         }
         temp = "HEIGHT";
...
async function insertToDB(data) {
   ...
   // let a = await Patient.findAll();
   // console.log(a);
   //Patient.upsert( patient[0])
   Patient.bulkCreate(patient, { updateOnDuplicate: ['from_healthkit_sync', 'orgId', 'timestamp'] })
      .then(result => {
         console.log('Inserted/Updated patients:', result);
         return ClinicalData.bulkCreate(report)
      })
      .then(result => {
         console.log('reports inserted', result);
      })
      .catch(err => {
...
function reportDBFormat(data, i, temp) {
   return ({
      "patient_id": data.patient_id,
      "test_type": temp,
      "test_name": data.clinical_data[temp].name,
      "uom": data.clinical_data[temp].uom,
      "value": data.clinical_data[temp].data[i].measurement,
      "on_date": data.clinical_data[temp].data[i].on_date ? new Date(data.clinical_data[temp].data[i].on_date) : null
   })
}
...
async function deletePatient(req, res) {
   try {
      ...
      if (result === 0) {
         console.log('No patient found with the provided patient_id.');
         return res.status(404).json({ message: 'No patient found with the provided patient_id: ' + req.params.patientId });
      } else {
         console.log('Patient deleted successfully.');
         return res.status(200).json({ message: `Patient ${req.params.patientId} and all his reports are deleted successfully.` });
...
async function deleteReport(req, res) {
   try {
      const result = await ClinicalData.destroy({
         where: {
            patient_id: req.params.patientId
         }
      });
      ...
      if (result === 0) {
         console.log('No report found with the provided patient_id.');
         return res.status(404).json({ message: 'No report found with the provided patient_id: ' + req.params.patientId });
      } else {
         console.log('Patient deleted successfully.');
         return res.status(200).json({ message: `Patient ${req.params.patientId} reports deleted successfully.` });
      }
   } catch (error) {
      console.error('Error deleting patient:', error);
      return res.status(500).json({ message: 'Error deleting patient reports' });
   }
}
...
async function getPatient(req, res) {
   try {
      ...
      if (result === 0) {
         ...
         return res.status(404).json({ message: 'Patient not found' + req.params.patientId || '' });
      } else {
...
async function getPatientAllReports(req, res) {
   try {
      const reports = await ClinicalData.findAll({
         where: {
            patient_id: req.params.patientId,
            value: {
               [Op.ne]: null,
         ...
         attributes: ['test_name', 'value', 'uom', 'on_date']
      });

      if (reports.length === 0) {
         return res.status(404).json({ message: 'No reports found for this patient. patient_id: ' + req.params.patientId });
      }
...
async function evaluatedHeartbeatReport(req, res) {
   try {
      const reports = await ClinicalData.findAll({
         where: {
            patient_id: req.params.patientId,
      ...
      let result = formatData(reports, false, 'value');
      return res.status(200).json(result);
...
async function clearAll(req, res) {
   try {
      await Promise.all([Patient.destroy({
         where: {

         }
      }),
      ClinicalData.destroy({
         where: {

         }
      })])
      console.log('all tables cleared');
      return res.status(200).json({ message: 'All tables cleared' });


   } catch (error) {
      console.error('Error clearing tables:', error);
      return res.status(500).json({ message: 'Error clearing tables.' });
   }
}

module.exports = { heartbeatReportRaw, deletePatient, deleteReport, getPatientAllReports, evaluatedHeartbeatReport, getPatient, clearAll };
